...
--
-- Client :  127.0.0.1
-- Généré le :  Lun 24 Novembre 2014 à 12:26
...
CREATE TRIGGER `AUTO_ADD_STOCK_LIV` AFTER INSERT ON `t_livrdetail_lid`
 FOR EACH ROW UPDATE t_stock_stk
INNER JOIN t_livrdetail_lid ON t_livrdetail_lid.pro_id = t_stock_stk.pro_id 
SET t_stock_stk.stk_stock = t_stock_stk.stk_stock-NEW.lid_quantite
WHERE t_stock_stk.pro_id = t_livrdetail_lid.pro_id
//
DELIMITER ;
